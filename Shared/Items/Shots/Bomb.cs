...
namespace Fanior.Shared
{
    public class Bomb : Shot
    {
        ...
        public override void Dispose()
        {
            if (gvars.server)
            {
                Character c = gvars.Items[CharacterId] as Character;
                for (int i = 0; i < 12; i++)
                {
                    new BasicShot(gvars, this.X, this.Y, new Shape("lightblue", "darkblue", 2, 10, 10, Shape.GeometryEnum.circle, "rgb(255, 20, 50)", "darkred"), new Mask(10, 10, Shape.GeometryEnum.circle), 20, c.Damage * 20, -1, 2 * Math.PI * i / 12, 0, 0.5, 60);
                }
            }
            base.Dispose();
        }
